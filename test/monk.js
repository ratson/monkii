...
test('connect with promise', t =>
  monk('127.0.0.1/monk-test').then(db => {
    t.true(db instanceof Manager)
  }))
...
test('should fail with promise', t =>
  monk('non-existent-db/monk-test').catch(err => {
    t.truthy(err)
  }))
...
test.cb('close > closing a closed connection should work with callback', t => {
  const db = monk('127.0.0.1/monk')
  db.then(() => t.is(db._state, 'open')).then(() =>
    db.close(() => {
...
test('Manager#listCollections', t =>
  db.listCollections().then(collections => t.true(Array.isArray(collections))))
